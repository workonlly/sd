            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <div style={{ minWidth: 0, flex: 1 }}>
                    ...
                    <div style={{ fontSize: 9, color: '#64748b', fontWeight: 500, marginTop: 2 }}>
                        ...
                        {isMale ? ' Â· Male' : isFemale ? ' Â· Female' : ''}
                    </div>
                </div>
            </div>

            {isRoot && (
                <div style={{ position: 'absolute', top: -8, right: -8, background: '#6366f1', color: '#fff', padding: '2px 6px', borderRadius: 6, fontSize: 8, fontWeight: 800 }}>â˜…</div>
            )}
...
function CanvasImpl() {
    ...
    const mergeDataIntoGraph = useCallback((newData: any, anchorId: string | null) => {
        setNodes(currNodes => {
            ...
            // No union nodes needed â€” couples connect directly
            ...
            const occupiedXByY = new Map<number, number[]>(); // y-bucket â†’ array of x positions used
            ...
            // 6. Build final node array â€” FREEZE existing positions, use new positions for brand-new
        ...
        // Merge Edges additively â€” never wipe existing edges
        setEdges(currEdges => {
            ...
            newData.families?.forEach((fam: any) => {
                // Direct husband â†” wife pink dashed edge (no union node needed)
